    ...
    return (
      ...
      >
        <Flex gap="20px" align="center">
            <Text as="b" fontSize="3xl" color={fontColor}>{String(competenceNumber)}</Text>
            <Flex direction="column" width="100%" gap="7px">
                <Text as="b" fontSize="lg" color={fontColor}>{competenceLevel}</Text>
                { startDate ? <Text fontSize="xs">Start Date: {startDate}</Text> : null}
                { endDate ? <Text fontSize="xs">End Date: {endDate}</Text> : null}
            </Flex>
        </Flex>
